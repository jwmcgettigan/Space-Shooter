...
public class ProjectileLauncher : MonoBehaviour {
    ...
    private void launch() {
        ...
        if (Time.time > fireRate + lastShot && pos.y < GameObject.Find("Top Boundary").transform.position.y) {
            GameObject bullet = (GameObject)Instantiate(projectile, new Vector2(pos.x, pos.y + nudge), transform.rotation);

            bullet.GetComponent<Rigidbody2D>().velocity = transform.TransformDirection(dir * speed);
            lastShot = Time.time;
            Destroy(bullet, 3.0f);
        }
    }
